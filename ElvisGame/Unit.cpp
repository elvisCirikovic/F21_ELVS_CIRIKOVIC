...
bool Unit::CollideWith(const Unit& other) const
{
	...
	int oYtop{ other.GetPosY() + other.GetUnitWidth() };
	bool intersectOnX{ (mPosX <= oXleft && oXleft <= mPosX + GetUnitWidth()) || (mPosX <= oXright && oXright <= mPosX + GetUnitWidth()) || (oXleft<= mPosX && mPosX+ GetUnitWidth()<= oXright)};

	bool intersectOnY{(mPosY<=oYbottom && oYbottom<=mPosY+GetUnitHeight()) || (mPosY<= oYtop && oYtop <= mPosY +GetUnitHeight()) || (oYbottom<= mPosY && mPosY + GetUnitHeight() <= oYtop)};

	return intersectOnX && intersectOnY;
}

void Unit::Draw(Mirage::Shader& shader)
{
	Mirage::Renderer::Draw(mImage, mPosX, mPosY, mImage.GetWidth(), mImage.GetHeight(), shader);
}
...
{
	if ((newX < 0) || (newY < 0) || (newX + mImage.GetWidth() > 800) || (newY + mImage.GetHeight() > 800)) 
		return false;
	else
		return true;
